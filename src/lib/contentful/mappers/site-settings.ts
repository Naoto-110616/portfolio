...
export function mapSiteSettingsEntry(entry: ContentfulEntry | undefined): SiteSettings {
  ...
  const parsedSiteSettings = siteSettingsSchema.safeParse({
    metadata: {
      ...
      description:
        asString(fields.siteDescription) ?? fallbackSiteSettings.metadata.description,
      siteUrl: asString(fields.siteUrl) ?? env.NEXT_PUBLIC_SITE_URL,
    },
    header: {
      sinceLabel:
        asString(fields.headerSinceLabel) ?? fallbackSiteSettings.header.sinceLabel,
    ...
    footer: {
      ...
      copyright:
        asString(fields.footerCopyright) ?? fallbackSiteSettings.footer.copyright,
      backToTopLabel:
        asString(fields.footerBackToTopLabel) ??
        fallbackSiteSettings.footer.backToTopLabel,

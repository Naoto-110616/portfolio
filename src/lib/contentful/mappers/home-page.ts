...
function mapAboutSection(entry: ContentfulEntry | undefined) {
  ...
  return {
    ...
    portraitImageUrl:
      getAssetUrl(portraitAsset) ?? fallbackHomePage.about.portraitImageUrl,
    portraitImageAlt:
...
export function mapHomePageEntry(entry: ContentfulEntry | undefined): HomePageContent {
  ...
  const parsedHomePage = homePageSchema.safeParse({
    ...
    chat: {
      ...
      description:
        asString(fields.chatDescription) ?? fallbackHomePage.chat.description,
      helperText:
        asString(fields.chatHelperText) ?? fallbackHomePage.chat.helperText,
      placeholder:
        asString(fields.chatPlaceholder) ?? fallbackHomePage.chat.placeholder,
    },
    about: mapAboutSection(aboutSection),
    services: mapServicesSection(servicesSection),
    sectionTitles: {
      work:
        asString(fields.workSectionTitle) ?? fallbackHomePage.sectionTitles.work,
      moreProjects:
        asString(fields.moreProjectsSectionTitle) ??
        fallbackHomePage.sectionTitles.moreProjects,
    },
    source: "contentful",
  });

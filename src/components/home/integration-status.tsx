...
function StatusPill({ label, isActive }: { label: string; isActive: boolean }) {
	return (
		<div className="flex items-center justify-between rounded-2xl border border-white/10 bg-white/5 px-4 py-3">
			<span className="text-sm text-slate-200">{label}</span>
			<span
				className={`rounded-full px-3 py-1 text-xs font-medium ${
					isActive ? "bg-emerald-400/15 text-emerald-200" : "bg-amber-400/15 text-amber-100"
				}`}
			>
	...
	return (
		<section className="rounded-[2rem] border border-white/10 bg-slate-950/40 p-8">
			<div className="flex flex-col gap-2">
				<p className="text-sm tracking-[0.3em] text-cyan-200/80 uppercase">TanStack Query</p>
				<h2 className="text-2xl font-semibold text-white">Integration status dashboard</h2>
				<p className="text-sm leading-6 text-slate-300">
					クライアント側では React Query を標準化し、環境変数の設定状況や API
					の動作確認をすぐに差し替えられるようにしています。
				</p>
			</div>
